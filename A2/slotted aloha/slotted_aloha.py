...
if __name__ == "__main__":
    ...
    parser.add_argument("-r", help="if enabled, the program quits if number of retransmissions for a particular packet exceeds 10", action = "store_true")

    args = parser.parse_args()

    window_sizes = [args.w]*args.n #current collision window size
    k = [0]*args.n #backoff counter
    retransmission_counter = [0]*args.n #retransmission counter
    packet_generation_times = [list() for i in range(args.n)] #stores generation times of packets 
    current_time = 0
    num_packets_transmitted = 0
    average_delay = 0 #maintained as a moving average
    ...
    while num_packets_transmitted < args.m:
        ...
        for i in range(args.n):
            ...
            if k[i] == 0 and len(packet_generation_times[i]) > 0:
                #packet transmitted
                retransmission_counter[i] += 1
                if args.r and retransmission_counter[i] > 10:
                    print("Program terminated as retransmission count exceeded for packet of node ",i)
                    os._exit(1)
        ...
        if len(transmitting_nodes) == 1:
            ...
            retransmission_counter[transmitting_nodes[0]] = 0
